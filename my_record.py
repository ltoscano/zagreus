...
class MyRecord:
  ...
  def process(self, file_p):
    ...
    for line in lines:
      ...
      preset_interval = self.onset - timedelta(seconds=interval_base)
      if (self.my_class == "VF_RR" or self.my_class == "VT_RR"):
        self.intervals.append((curr_time, float(words[2])))
      elif (self.onset > curr_time and curr_time > preset_interval):
        self.intervals.append((curr_time, float(words[2])))
  ...
  def prune_vt_vf(self):
    # onset is at the very end of the file
    # only keep the very last preset interval
    pruned_intervals = list()
    last_interval = self.intervals[-1][0]
    preset_interval = last_interval - timedelta(seconds=interval_base)
    for (curr_time, interval) in self.intervals:
      if (last_interval > curr_time and curr_time > preset_interval):
        pruned_intervals.append((curr_time, interval))
    self.intervals = pruned_intervals
    self.my_class = "SCD"
